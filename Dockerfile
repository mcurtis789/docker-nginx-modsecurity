...
RUN  cd ~ && \
  ...
  cd certbot && \
  CFLAGS="-I/usr/local/include" LDFLAGS="-L/usr/local/lib" /usr/bin/pip install -r ./certbot/readthedocs.org.requirements.txt && \
...
RUN  mkdir -p /etc/nginx/modsec/conf.d && \
  ...
  echo -e "# Example placeholder\n" > /etc/nginx/modsec/conf.d/example.conf && \
  echo -e "# Include the recommended configuration\nInclude /etc/nginx/modsec/modsecurity.conf\n# User generated\nInclude /etc/nginx/modsec/conf.d/*.conf\n\n# OWASP CRS v${MODSECURITY} rules\nInclude /usr/local/coreruleset-${OWASPCRS_VERSION}/crs-setup.conf\nInclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/*.conf\n" > /etc/nginx/modsec/main.conf && \
  echo -e "# For inclusion and centralized control\nmodsecurity on;\n" > /etc/nginx/modsec/modsec_on.conf && \
  echo -e "# For inclusion and centralized control\nmodsecurity_rules_file /etc/nginx/modsec/modsec_includes.conf;\n" > /etc/nginx/modsec/modsec_rules.conf
  #echo -e "# For inclusion and centralized control\ninclude /etc/nginx/modsec/modsecurity.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/crs-setup.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/REQUEST-900-EXCLUSION-RULES-BEFORE-CRS.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/REQUEST-901-INITIALIZATION.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/REQUEST-905-COMMON-EXCEPTIONS.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/REQUEST-911-METHOD-ENFORCEMENT.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/REQUEST-912-DOS-PROTECTION.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/REQUEST-913-SCANNER-DETECTION.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/REQUEST-920-PROTOCOL-ENFORCEMENT.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/REQUEST-921-PROTOCOL-ATTACK.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/REQUEST-930-APPLICATION-ATTACK-LFI.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/REQUEST-931-APPLICATION-ATTACK-RFI.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/REQUEST-932-APPLICATION-ATTACK-RCE.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/REQUEST-933-APPLICATION-ATTACK-PHP.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/REQUEST-941-APPLICATION-ATTACK-XSS.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/REQUEST-942-APPLICATION-ATTACK-SQLI.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/REQUEST-943-APPLICATION-ATTACK-SESSION-FIXATION.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/REQUEST-949-BLOCKING-EVALUATION.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/RESPONSE-950-DATA-LEAKAGES.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/RESPONSE-951-DATA-LEAKAGES-SQL.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/RESPONSE-952-DATA-LEAKAGES-JAVA.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/RESPONSE-953-DATA-LEAKAGES-PHP.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/RESPONSE-954-DATA-LEAKAGES-IIS.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/RESPONSE-959-BLOCKING-EVALUATION.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/RESPONSE-980-CORRELATION.conf\ninclude /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/RESPONSE-999-EXCLUSION-RULES-AFTER-CRS.conf\n" > /etc/nginx/modsec/modsec_includes.conf && \

RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-900-EXCLUSION-RULES-BEFORE-CRS.conf" > /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-901-INITIALIZATION.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-905-COMMON-EXCEPTIONS.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-911-METHOD-ENFORCEMENT.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-913-SCANNER-DETECTION.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-920-PROTOCOL-ENFORCEMENT.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-921-PROTOCOL-ATTACK.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-922-MULTIPART-ATTACK.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-930-APPLICATION-ATTACK-LFI.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-931-APPLICATION-ATTACK-RFI.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-932-APPLICATION-ATTACK-RCE.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-933-APPLICATION-ATTACK-PHP.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-934-APPLICATION-ATTACK-GENERIC.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-941-APPLICATION-ATTACK-XSS.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-942-APPLICATION-ATTACK-SQLI.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-943-APPLICATION-ATTACK-SESSION-FIXATION.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-944-APPLICATION-ATTACK-JAVA.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/REQUEST-949-BLOCKING-EVALUATION.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/RESPONSE-950-DATA-LEAKAGES.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/RESPONSE-951-DATA-LEAKAGES-SQL.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/RESPONSE-952-DATA-LEAKAGES-JAVA.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/RESPONSE-953-DATA-LEAKAGES-PHP.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/RESPONSE-954-DATA-LEAKAGES-IIS.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/RESPONSE-955-WEB-SHELLS.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/RESPONSE-959-BLOCKING-EVALUATION.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/RESPONSE-980-CORRELATION.conf" >> /etc/nginx/modsec/modsec_includes.conf
RUN echo "include /usr/local/coreruleset-4.3.0/rules/RESPONSE-999-EXCLUSION-RULES-AFTER-CRS.conf" >> /etc/nginx/modsec/modsec_includes.conf

RUN  mv /src/modsecurity.conf /etc/nginx/modsec && \
  ...
  #ls -al /usr/local/coreruleset-${OWASPCRS_VERSION}/rules/ \
  ...
  cp /usr/local/coreruleset-${OWASPCRS_VERSION}/crs-setup.conf.example /usr/local/coreruleset-${OWASPCRS_VERSION}/crs-setup.conf && \
